echo off
bcdedit.exe /enum {current} | findstr "safeboot" >nul && goto STARTDELETE || goto SETSAFEMODE

:SETSAFEMODE
bcdedit.exe /set {current} safeboot minimal >nul
echo Setting safeboot mode has been done.
goto REBOOT
...
:REBOOT
set /p rbt="Ready to reboot? (Y/n): "
if NOT "%rbt%"=="n" shutdown /r /t 1
